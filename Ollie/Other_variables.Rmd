...
data_csv_test <- data$test
data_csv_train <- data$train
...
data <- data.frame(data_csv_train[,c("winner_id", "winner_name", "winner_hand", "winner_ht", "w_ace", "w_df", "w_svpt", "w_1stIn", "w_1stWon",
...
model <- BTm(player1=winners, player2=losers,
                  formula = ~  Hand[Player] + Height[Player] + Surface + Ace + Svpt + (1|Player),
...
```{r}
predictions <- predict_matches(data_csv_test, players, model)
