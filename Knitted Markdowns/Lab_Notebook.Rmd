...
Cov_Results<-matrix(NA,nrow=11,ncol=3)
...
Cov_Results[1,2]<-length(which(pred0$fit>0.5))
Cov_Results[1,3]<-length(which(pred0$fit>0.5)) / length(pred0$fit)

Cov_Results[2,1]<-mean(pred_full$fit)
Cov_Results[2,2]<-length(which(pred_full$fit>0.5))
Cov_Results[2,3]<-length(which(pred_full$fit>0.5)) / length(pred_full$fit)
...
for (i in 1:9){
  ...
  Cov_Results[i+2,2]<-length(which(pred$fit>0.5))
  Cov_Results[i+2,3]<-length(which(pred$fit>0.5)) / length(pred$fit)
}
