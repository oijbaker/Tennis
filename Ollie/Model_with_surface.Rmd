...
count <- count(all_players) # counts number of rows each unique name has in all_players, i.e. each unique player's number of total wins & losses
players <- filter(count, freq >= 20) # extracts players who have played 20+ matches
```
...
```{r}
model_3vars <- BTm(1, Winner, Loser, ~ W_Hand[..] + L_Hand[..] + W_Height[..] + L_Height[..] + Surface[..] + (1|..), data = data_for_model)
model_3vars
summary(model_3vars)
```
```{r}
abilities <- as.data.frame(BTabilities(model_3vars))
abilities[order(-abilities$ability),]
```